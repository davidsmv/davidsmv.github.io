...
function renderBooks(containerId, limit = null) {
    ...
    container.innerHTML = dataToRender.map(book => {
        if (book.goodreadsHtml) {
            // Extract the specific review URL if possible
            const match = book.goodreadsHtml.match(/href="(https:\/\/www\.goodreads\.com\/review\/show\/[^"]+)"/);
            const reviewUrl = match ? match[1] : null;

            // Remove the "View all my reviews" link and trailing breaks to clean up the UI
            let cleanHtml = book.goodreadsHtml.replace(/<br\s*\/?>\s*<br\s*\/?>\s*<a href="https:\/\/www\.goodreads\.com\/review\/list\/[^"]+">View all my reviews<\/a>/i, '');
            
            // Fallback: just remove the link if the BR tags are different
            cleanHtml = cleanHtml.replace(/<a href="https:\/\/www\.goodreads\.com\/review\/list\/[^"]+">View all my reviews<\/a>/i, '');

            return `
                <div class="list-item" style="display: block;">
                    <div class="book-content collapsed">
                        ${cleanHtml}
                    </div>
                    ${reviewUrl ? `
                    <div style="text-align: center; margin-top: 5px;">
                        <a href="${reviewUrl}" target="_blank" class="read-more-btn" style="text-decoration: none; display: inline-block;">Read Full Review on Goodreads</a>
                    </div>
                    ` : ''}
                </div>
            `;
...
function renderContact(containerId) {
    ...
    container.innerHTML = contactData.map(contact => `
        <div class="project-card" style="text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center;">
            <i class="${contact.icon}" style="font-size: 3rem; color: ${contact.color}; margin-bottom: 1rem;"></i>
            <h3>${contact.platform}</h3>
            <a href="${contact.link}" target="_blank" class="project-link" style="margin-top: 0.5rem; color: ${contact.color}; border: 1px solid ${contact.color}; padding: 5px 15px; border-radius: 5px;">
                ${contact.text}
            </a>
        </div>
    `).join('');
...
function renderFooter(containerId) {
    const container = document.getElementById(containerId);
    if (!container || typeof contactData === 'undefined') return;

    const iconsHtml = contactData.map(contact => 
        `<a href="${contact.link}" target="_blank" title="${contact.platform}" class="footer-icon" style="color: white; margin: 0 10px; font-size: 1.5rem; transition: color 0.3s; text-decoration: none;">
    ...
    container.innerHTML = `
        ...
        <p>&copy; ${new Date().getFullYear()} David Martínez. All rights reserved.</p>
    `;
}
